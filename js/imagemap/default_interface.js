...
function gui_row_select(id, setfocus, multiple) {
	...
	//put highlight on actual props row 선택 행 컬러 지정
...
function gui_addArea(id) {
	...
	// 이미지 좌표 영역 체크박스
	var temp = '<label><input type="radio" name="img_active" class="img_active" id="img_active_' + id + '" value="' + id + '"></label>';

	// 이미지 좌표 번호 ID 값
	temp += '<input type="text"  id="img_id" class="img_id" value="' + (id) + '" readonly="1" />';
	...
	// 이미지 좌표 필드 명
	temp += '<label class="field_name">&nbsp 필드 명: </label>';
	temp += '<input type="text" name="img_alt" class="form-control" value="">';
	...
	// 좌표 id 값
	temp += '<br><input type="text" name="xy_id" class="xy_id" value="" style="display: none;">';

	// OCR 결과 값
	// temp += '<br>텍스트: <input type="text" name="ocr_result" class="ocr_result" value="" ">';
...
function gui_modeChanged(mode) {
	...
	if (mode == 1) {
		//preview mode
		if (document.getElementById('tatainer')) {
...
function gui_loadImage(src, type) {
	// 템플릿 선택 초기화
